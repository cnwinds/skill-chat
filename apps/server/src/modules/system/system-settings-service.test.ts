...
describe('SystemSettingsService', () => {
  ...
  it('keeps persisted settings for mutable runtime config while WEB_ORIGIN stays env-managed', async () => {
    ...
    expect((settings as { webOrigin?: string }).webOrigin).toBeUndefined();
    ...
    expect(rows.has('web_origin')).toBe(false);
  ...
  it('keeps persisted settings in production while WEB_ORIGIN stays env-managed', async () => {
    const { config, db } = await createDb({
      NODE_ENV: 'production',
      WEB_ORIGIN: 'https://env.example.com',
      OPENAI_MODEL: 'gpt-5.4',
      OPENAI_REASONING_EFFORT: 'medium',
      ENABLE_ASSISTANT_TOOLS: true,
    });
    ...
    expect(config.ENABLE_ASSISTANT_TOOLS).toBe(false);
    expect(config.WEB_ORIGIN).toBe('https://env.example.com');
    expect(config.OPENAI_MODEL).toBe('gpt-5.3');
    expect(config.OPENAI_REASONING_EFFORT).toBe('low');
  });
