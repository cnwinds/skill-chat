...
- `HARNESSKIT_VERSION` 必须与已发布到 npm 的 `@skillchat/harness-*` 版本一致；Docker 构建时会自动切换为 registry 依赖（见 `docs/Harness_Kit_Workflow.md`）
- `@skillchat/*` 仅发布在 `registry.npmjs.org`；Dockerfile 已对该 scope 单独指定官方源，其余包仍可用 `NPM_REGISTRY`（默认 npmmirror）
